...
namespace DemoAddIn
{
        ...
        
    {
        ...
        async void ISEMouseEvents.MouseUp(short sButton, short sShift, double dX, double dY, double dZ, object pWindowDispatch, int lKeyPointType, object pGraphicDispatch)
        {
            ...
            HoleInfo _holeInfo2 = default(HoleInfo);

             var bb = _doc.RefPlanes.Item(1); 
             Array norms = new double[3] as Array;
             bb.GetNormal(ref norms);
             double[] nss = norms as double[];

             _holeInfo2.nx = nss[0];
             _holeInfo2.ny = nss[1];
             _holeInfo2.nz = nss[2];

             MessageBox.Show($"Plane_dir {_holeInfo2.nx}, {_holeInfo2.ny}, {_holeInfo2.nz}");
